...
import 'package:mnd_shop/core/media/offer_image_crop_page.dart';
import 'package:mnd_shop/core/media/offer_image_spec.dart';
import 'package:mnd_shop/core/media/shop_cover_image_picker.dart';

/// Picks a gallery image and crops to the customer app home offer banner aspect ratio.
class OfferImagePicker {
  OfferImagePicker._();
  ...
  static Future<Uint8List?> pickFromGalleryAndCrop(BuildContext context) async {
    final bool permitted =
        await ShopCoverImagePicker.ensurePhotoLibraryPermission();
    if (!permitted) {
      if (context.mounted) {
    ...
    final Uint8List? cropped = await OfferImageCropPage.open(
      context,
      imageBytes: raw,
    );
    if (cropped == null) {
    ...
    return OfferImageSpec.encodeForUpload(cropped);
  }
}
