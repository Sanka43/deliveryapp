...
class _VendorProfilePageState extends ConsumerState<VendorProfilePage> {
  ...
  bool _saving = false;
  List<String> _baselineFields = const <String>[];

  List<String> _currentFields() => <String>[
    _name.text,
    _description.text,
    _phone.text,
    _whatsapp.text,
    ...
    _close.text,
    _openingNote.text,
    _closedSunday.toString(),
  ];

  bool get _isDirty =>
      _hydrated && !listEquals(_currentFields(), _baselineFields);

  static final RegExp _timeOfDayPattern = RegExp(r'^([01]\d|2[0-3]):[0-5]\d$');

  static String? _validateTimeOfDay(String? v) {
    final String t = (v ?? '').trim();
    if (t.isEmpty) {
      return 'Required';
    }
    if (!_timeOfDayPattern.hasMatch(t)) {
      return 'Use 24-hour HH:MM, e.g. 09:00';
    }
    return null;
  }
  ...
  Future<bool> _confirmDiscardChanges(BuildContext context) async {
    final bool? leave = await showDialog<bool>(
      ...
      builder: (BuildContext ctx) => AlertDialog(
        ...
        content: const Text(
          'You have unsaved changes to your shop profile. Leave without saving?',
        ),
        actions: <Widget>[
          TextButton(
  ...
  void _hydrateFromDoc(Map<String, dynamic>? doc) {
    ...
    _baselineFields = _currentFields();
  }
  ...
  Future<void> _save() async {
    ...
    if (err != null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            userFacingError(err, fallback: 'Could not save. Please try again.'),
          ),
        ),
      );
      return;
  ...
  Widget build(BuildContext context) {
    ...
    final String? authEmail = ref
        .watch(firebaseAuthProvider)
        .currentUser
        ?.email
        ?.trim();
    final String email = (authEmail != null && authEmail.isNotEmpty)
        ? authEmail
        : 'No email set';
    ...
    return PopScope(
      // Always intercept and decide fresh in the callback — text field edits
      // don't trigger a rebuild here, so a `canPop` computed once in build()
      // would go stale the moment the vendor typed anything.
      canPop: false,
      onPopInvokedWithResult: (bool didPop, Object? result) async {
        ...
        final bool leave = await _confirmDiscardChanges(context);
