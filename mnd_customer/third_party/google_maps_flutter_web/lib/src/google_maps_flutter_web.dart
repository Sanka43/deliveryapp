...
class GoogleMapsPlugin extends GoogleMapsFlutterPlatform {
  ...
  void dispose({required int mapId}) {
    // `google_maps_flutter`'s `_GoogleMapState._disposeController()` awaits
    // the controller future before calling `dispose()` on it (fire-and-
    // forget, from `State.dispose()`), which can race with another dispose
    // for the same [mapId] (e.g. two quick picker push/pops) — the second
    // call used to assert on a controller that's already been removed here,
    // surfacing as an uncaught async error and leaving the map's overlays
    // (markers/route polyline) in a stale state. A missing controller means
    // there's simply nothing left to dispose.
    final GoogleMapController? controller = _mapById[mapId];
    if (controller == null) {
      return;
    }
